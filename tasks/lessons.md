...
- Full replay launched from run recap must preserve a return destination. Separately, "Tap to replay" means the completed five-second death-clip control, not the game-over Retry button; confirm the exact labeled interaction before changing adjacent navigation.
...
- Stopping a Web Audio source does not prove Safari released its graph. Every transient source, filter, gain, and oscillator connected to a long-lived master must be explicitly disconnected on a bounded lifecycle, and device diagnostics should report the live node count at wave boundaries.
- A plausible leak plus a passing desktop cleanup probe is not a device root-cause confirmation. Call it one owner until the same iPhone workload survives; when failure remains, use the new cutoff evidence to revise the model before stacking more speculative cleanup.
- An earlier device cutoff can make a memory bug easier to reproduce without proving a regression. Compare wave depth, resource high-water marks, and the last completed operation; separately remove any new unproven trigger so the reliable repro does not become a second bug.
- A higher failure threshold validates a removed pressure source but does not satisfy a memory-fix acceptance test. Keep the successful reduction, then use the new terminal event and surviving stress cases to isolate the next owner until the device workload stops restarting.
- When the same termination appears during ordinary replay playback, do not assume game-over overlap is the sole cause. First determine whether replay started fresh or inherited the prior run's renderer; lifecycle fixes must explain every confirmed repro surface.
- A mid-replay failure does not prove a fresh replay leaks. Inspect what ran earlier in the same boot: if replay inherits the prior human run's renderer high-water pools, the correct boundary is renderer ownership between runs, not replay simulation behavior.
- Chromium successfully recreating a WebGL renderer on a canvas does not certify that lifecycle on iOS WKWebView. A replacement context can keep simulation and audio alive while rendering a blank canvas; preserve the device-proven primary context and release owned scene resources in place.
- A cleanup that makes the first full replay survive can still make a second replay fail if it destroys and reallocates the same bounded GPU scene every time. Distinguish human-to-replay cleanup from replay-to-identical-replay reuse; on WKWebView, logical destruction does not prove immediate driver-memory reclamation.
