...
('A · Analysis AI','Reproduce baseline evidence.'),('B · Implementation AI','Build shared accounting and missile flank pools.'),('C · Implementation AI','Add drone cruise/commit/dive routing.'),('D · Independent AI','Verify the combined implementation.'),('E · You','Feel-check both threat families together.'),('F · Coordinating AI','Document results and remaining limitations.')], 'execution')+'<div class="callout">Routing direction agreed → settle the remaining contract decisions inside the detailed AI execution handoff → implement. No implementation has started.</div>', 'Stages are sequential gates. D returns defects to B/C; E checks the combined experience. Different sessions may own different stages. The open contract rules are settled in the execution handoff that precedes stage B, not in a stage of their own.'),
...
OUTPUT.write_text('''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>Effective target pressure — review plan</title><style>'''+css+'''</style></head><body><header><div class="eyebrow">Dubai Missile Command · Design review · 20 September 2026</div><h1>Make every defense<br>decision matter.</h1><p>Budget the attacks that actually threaten the tower. Give buildings and combat infrastructure a meaningful share of the battle.</p><span class="badge">HISTORICAL REVIEW · STAGE B IMPLEMENTED</span><div class="top-tools"><a href="#6-review-decisions-requested">Jump to review decisions ↓</a><a href="target-pressure-review-plan.md">Markdown source</a><button type="button" onclick="window.print()">Print / save PDF</button></div></header><div class="layout"><nav aria-label="Plan sections"><p>In this plan</p>'''+nav+'''</nav><main><div class="companion">Illustrated companion to the review plan. Missile flank pools and drone cruise/commit/dive routing are agreed. Stage B is implemented; see <a href="target-pressure-stage-b-execution.md">accepted rules, evidence and Stage C handoff</a>. The review questions below are historical. <a href="../ROADMAP.html#rm-10">ROADMAP.html</a> remains the source of product status.</div>'''+''.join(body)+'''</main></div><footer>Self-contained HTML · Ten diagrams · No external fonts, libraries or network requests.<br>Generated from <a href="target-pressure-review-plan.md">the Markdown review plan</a> with <code>scripts/docs/render-target-pressure-plan.py</code>. Regenerate after source edits.</footer></body></html>''')
print(OUTPUT)
