...
- **Report schema v1:** `{ schemaVersion: 1, runId, buildId, replayId, deviceInfo: { ua, dpr, drawingBufferSize, screenSize, isCapacitor }, frames: [{ tick, frameMs, gpuMs?, presentMs?, missiles, drones, interceptors, particles, explosions }], gpuProfile?: { source, captureMode, frameCount, gpuSummary, presentSummary?, notes? }, summary: { p50, p95, p99, longFrameCount16, longFrameCount33 } }`. Versioned from day one. `gpuMs`/`presentMs` on frames and `gpuProfile` are populated post-hoc by the smoke harness via Chromium trace merge, never by the in-browser recorder.
