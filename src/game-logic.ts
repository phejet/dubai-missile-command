...
export function hitsBurjBody(g: GameState, x: number, y: number): boolean {
  return (
    ...
    y <= GAMEPLAY_SCENIC_THREAT_FLOOR_Y &&
    Math.abs(x - BURJ_X) <= getGameplayBurjHalfW(y, 2)
  );
}

// Missiles that reach the ground still damage the Burj when they were aimed at it.
...
export function predictBurjImpactTicks(g: GameState, t: Threat, horizon: number): number | null {
  ...
  if (t.type === "drone") return predictDroneBurjImpact(g, t, horizon);
  return predictMissileBurjImpact(g, t, horizon);
}
...
function predictMissileBurjImpact(g: GameState, m: Missile, horizon: number): number | null {
  ...
  for (let tick = 1; tick <= horizon; tick++) {
    ...
    if (y >= GAMEPLAY_WATERLINE_Y) return isBurjImpactTarget(m.targetX, m.targetY) ? tick : null;
...
function predictDroneBurjImpact(g: GameState, d: Drone, horizon: number): number | null {
  if (d.waypoints && d.waypoints.length >= 2) {
    ...
    for (let tick = 1; tick <= horizon; tick++) {
      ...
      if (d.diveTarget) {
        if (dist(x, y, d.diveTarget.x, d.diveTarget.y) < 20 || y >= GAMEPLAY_WATERLINE_Y || pathDone) {
          return isBurjDiveTarget(g, d) ? tick : null;
        }
  ...
  for (let tick = 1; tick <= horizon; tick++) {
    ...
    if (x < -60 || x > CANVAS_W + 60 || y > CANVAS_H + 20) return null;
    if (hitsBurjBody(g, x, y)) return tick;
    if (d.diveTarget && (dist(x, y, d.diveTarget.x, d.diveTarget.y) < 20 || y >= GAMEPLAY_WATERLINE_Y)) {
      return isBurjDiveTarget(g, d) ? tick : null;
    }
