...
export function drawGame(ctx, game, { showShop = false } = {}) {
  let sx = 0,
    sy = 0;
  ...
  game.missiles.forEach((m) => {
    ...
    if (m.type === "mirv") {
      ...
      // Pulsing red glow
      glow(ctx, "#ff2200", 15 + Math.sin(game.time * 0.2) * 5);
      ...
      ctx.rotate(angle);
      // Pulsing glow
      glow(ctx, "#dd4422", 8 + Math.sin(game.time * 0.4) * 3);
      // Body
  ...
  const activeUpgrades = Object.entries(game.upgrades).filter(([, v]) => v > 0);
  ...
  if (game.waveComplete && game.waveClearedTimer > 0) {
    ...
    ctx.font = "bold 22px 'Courier New', monospace";
    ctx.fillStyle = COL.hud;
    ctx.fillText(`WAVE ${game.wave} CLEARED`, CANVAS_W / 2, 312);
    ctx.textAlign = "left";
    ctx.restore();
  }
}
...
export function drawGameOver(ctx, finalScore, finalWave, finalStats) {
  const t = performance.now() / 1000;
