...
describe("getWaveConfig", () => {
  it("returns valid config for waves 1-12", () => {
    for (let w = 1; w <= 12; w++) {
      ...
      for (const type of ["missile", "drone136", "drone238", "mirv"]) {
...
describe("advanceSpawnSchedule", () => {
  function makeGameWithSchedule(schedule, cap = 100) {
    return {
      schedule,
      scheduleIdx: 0,
      waveTick: 0,
  ...
  it("resumes after cap drops", () => {
    ...
    const spawnFn = (_g, type) => {
      spawned.push(type);
      _g.missiles.push({ alive: true, type: "missile" });
    };
...
describe("isWaveFullySpawned", () => {
  it("false when schedule not exhausted", () => {
    expect(isWaveFullySpawned({ schedule: [{ tick: 0 }], scheduleIdx: 0 })).toBe(false);
  });

  it("true when schedule exhausted", () => {
    expect(isWaveFullySpawned({ schedule: [{ tick: 0 }], scheduleIdx: 1 })).toBe(true);
  });
});
...
describe("commanderPickTactics", () => {
  ...
  it("adaptive style avoids repeating tactics from last 2 waves", () => {
    ...
    cmdr.history = [
      { wave: 3, tactics: ["LEFT_FLANK"], budget: 28 },
      { wave: 4, tactics: ["RIGHT_FLANK", "LOW_APPROACH"], budget: 38 },
    ];
