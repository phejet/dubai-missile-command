...
export interface PixiBurjBaseHealthFloor {
  y: number;
  h: number;
  cx: number;
  halfW: number;
...
export interface PixiBurjBaseHealthLayout {
  ...
  frameW: number;
  frameH: number;
  floors: PixiBurjBaseHealthFloor[];
}
...
function drawBurjFoundation(graphic: Graphics, towerBaseY: number, groundY: number): void {
  void groundY;

  graphic
    .moveTo(BURJ_X - 96, towerBaseY + 2)
    .lineTo(BURJ_X - 82, towerBaseY - 12)
    .lineTo(BURJ_X - 56, towerBaseY - 15)
    .lineTo(BURJ_X - 34, towerBaseY - 8)
    .lineTo(BURJ_X - 16, towerBaseY - 3)
    .lineTo(BURJ_X + 16, towerBaseY - 3)
    .lineTo(BURJ_X + 34, towerBaseY - 8)
    .lineTo(BURJ_X + 56, towerBaseY - 15)
    .lineTo(BURJ_X + 82, towerBaseY - 12)
    .lineTo(BURJ_X + 96, towerBaseY + 2)
    .fill({ color: 0x161c28, alpha: 0.9 });

  graphic.rect(BURJ_X - 78, towerBaseY - 9, 156, 0.8).fill({ color: 0x2a3344, alpha: 0.32 });
  graphic.rect(BURJ_X - 88, towerBaseY - 4, 176, 0.8).fill({ color: 0x2a3344, alpha: 0.32 });
}
...
export function getPixiBurjBaseHealthLayout(towerBaseY = GAMEPLAY_TOWER_BASE_Y): PixiBurjBaseHealthLayout {
  ...
  const frameY = topFloor.y - 2;
  const frameH = bottomFloor.y + bottomFloor.h + 2 - frameY;
  const maxHalfW = floors.reduce((acc, floor) => Math.max(acc, floor.halfW), 0);
  const frameX = BURJ_X - maxHalfW;
  const frameW = maxHalfW * 2;
...
function createBurjFireParticleSystem(
  ...
  const flameCoreContainer = new ParticleContainer({
    dynamicProperties: { position: true, scale: true, rotation: true, color: true },
    boundsArea: new Rectangle(0, 0, CANVAS_W, CANVAS_H),
  });
  flameCoreContainer.label = "burj-fire-cores";
...
export class PixiRenderer implements GameRenderer {
  ...
  private buildGameplayScene(): void {
    ...
    const burjSmokeOverlay = new Graphics();
  ...
  private createGameplayGroundDecor(): Container {
    ...
    drawBurjFoundation(foundation, towerBaseY, groundY);
  ...
  private updateGameplayOverlays(
    ...
    this.updateBurjBaseHealthBar(state.burjBaseHealthBar, state.burjSmokeOverlay, game, sceneTime);
  ...
  private updateBurjBaseHealthBar(graphic: Graphics, smokeGraphic: Graphics, game: GameState, sceneTime: number): void {
    graphic.clear();
    smokeGraphic.clear();
    ...
    layout.floors.forEach((floor, floorIndex) => {
      ...
      if (isJustLost) {
        ...
        const stripeW = floor.halfW * 2;
        const ignitePulse = 0.55 + 0.45 * Math.sin(sceneTime * 13);
        graphic
          .rect(stripeLeftX - 1.6, floor.y - 1.4, stripeW + 3.2, floor.h + 2.8)
          .fill({ color: 0xfff0c8, alpha: flashT * (0.32 + ignitePulse * 0.2) })
          .stroke({ color: 0xffd080, alpha: flashT * 0.7, width: 1 });
      }
    });
  ...
  private updateBurjFireParticles(state: GameplayDynamicState, game: GameState, interpolationAlpha = 1): void {
    ...
    for (const particle of game.particles) {
      ...
      if (type === "fireSmoke" || type === "smokePuff" || type === "explosionPuff") {
        ...
        const baseRotation = particle.angle ?? 0;
        particleNode.rotation = baseRotation + Math.sin(timeS * 3.1 + phase) * 0.08;
        // Flames fade in very fast then ride high alpha until death.
        const alphaCurve = Math.min(1, lifeT * 2.6);
        particleNode.alpha = Math.min(1, alphaCurve * flameAlphaMul * 0.88);
        particleNode.tint = memoCssColorToNumber(particle.color, 0xffb84a);
        coreNode.x = pos.x + Math.sin(phase) * baseScale * 2.5;
        coreNode.y = pos.y + baseScale * 3.2;
        coreNode.scaleX = baseScale * 0.52 * flickerX;
        coreNode.scaleY = baseScale * 0.72 * flickerY;
        coreNode.rotation = particleNode.rotation;
        coreNode.alpha = Math.min(1, alphaCurve * flameAlphaMul);
        coreNode.tint = 0xfff0bc;
      }
    }

    trimParticlePool(system.flamePool, system.flameContainer, flameCount);
    trimParticlePool(system.flameCorePool, system.flameCoreContainer, flameCoreCount);
