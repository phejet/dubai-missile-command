...
function replayToCompletion(seed: number, actions: ReplayAction[], options: Record<string, unknown> = {}) {
...
describe("runGame recording", () => {
  ...
  it("recorded actions have valid structure", () => {
    ...
    for (const action of r.actions!) {
      ...
      expect(["fire", "shop", "emp", "f15", "flare", "wave_plan", "cursor"]).toContain(action.type);
      if (action.type === "fire") {
...
describe("golden-seed canary", () => {
  ...
  it("seed 42 at 5000 ticks produces expected draft-mode score and wave", () => {
    ...
    // 25904 -> 33892 (player-only scoring, replay v14): same run (wave 7, timeout), only
