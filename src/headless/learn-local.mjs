...
function benchmark(numGames, config) {
  const results = [];
...
async function queryQwen(model, config, stats, history, round) {
...
function applyPatch(patch, origConfig, origBrain) {
...
function revertPatch(origConfig, origBrain) {
  writeFileSync(CONFIG_PATH, origConfig);
  writeFileSync(BRAIN_PATH, origBrain);
}

function autoCommit(baseline, best, rounds) {
  try {
    ...
    console.log(`\nNothing to commit (no improvements kept) — ${e.message.slice(0, 80)}`);
  }
}
...
async function main() {
  ...
  for (let round = 1; round <= ROUNDS; round++) {
    ...
    let patch;
    try {
      patch = await queryQwen(model, JSON.parse(origConfig), best, history, round);
    } catch (err) {
      console.log(`  ✗ LLM error: ${err.message} — skipping round`);
      appendLog({ type: "error", round, error: err.message });
      continue;
    }
    ...
    // Apply and benchmark
    applyPatch(patch, origConfig, origBrain);
