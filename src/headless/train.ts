...
const NUM_GAMES = parseInt(getArg("games", "100") as string);
const NUM_ITERATIONS = parseInt(getArg("iterations", "10") as string);
const NUM_WORKERS = parseInt(getArg("workers", String(Math.min(8, (await import("os")).cpus().length))) as string);
const MAX_TICKS = parseInt(getArg("maxTicks", "100000") as string);
...
function runBatch(config: Record<string, unknown>, numGames: number) {
...
function aggregateStats(results: GameResult[]) {
...
async function main() {
  ...
  console.log(`Preset: ${PRESET || "draft"} (draft)`);
  console.log();
  ...
  for (let iter = 1; iter <= NUM_ITERATIONS; iter++) {
    ...
    const results = (await runBatch(config, NUM_GAMES)) as GameResult[];
