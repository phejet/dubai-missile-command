...
function analyzeWave(wave) {
  ...
  for (let s = 0; s < NUM_SAMPLES; s++) {
    ...
    threatValues.push(m * THREAT_VALUES.missile + d1 * THREAT_VALUES.drone136 + d2 * THREAT_VALUES.drone238 + mv * THREAT_VALUES.mirv);
  ...
  return {
    ...
    threatValue: { avg: avg(threatValues).toFixed(1), p10: pct(threatValues, 0.1).toFixed(1), p90: pct(threatValues, 0.9).toFixed(1) },
    budget: budgets[0],
...
console.log("Wave | Budget | Cap | Missiles  | Drone136  | Drone238  |   MIRV   | TotalThreats | ThreatValue | LastSpawnTick");
console.log("-----|--------|-----|-----------|-----------|-----------|----------|--------------|-------------|---------------");
for (const r of results) {
  ...
  console.log(`  ${String(r.wave).padStart(2)} |   ${String(r.budget).padStart(3)}  | ${String(r.concurrentCap).padStart(3)} | ${m.padEnd(9)} | ${d1.padEnd(9)} | ${d2.padEnd(9)} | ${mv.padEnd(8)} | ${tt.padEnd(12)} | ${tv.padEnd(11)} | ${lst}`);
}
