...
const ROUNDS = parseInt(getArg("rounds", "3") as string);
const DURATION_MS = parseInt(getArg("duration", "10000") as string);
...
async function main() {
  ...
  for (let round = 1; round <= ROUNDS; round++) {
    ...
    try {
      ...
      appendFileSync(
        LOG_PATH,
        JSON.stringify({ type: "error", round, timestamp: new Date().toISOString(), error: e.message }) + "\n",
      );
    }
  }
