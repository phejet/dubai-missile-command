import { CANVAS_W, LAUNCHERS, GROUND_Y, getRng } from "../game-logic";
import type { GameState, Threat, BotHumanState, Drone } from "../types";
...
function deepMerge(target: BotConfig, source: BotConfig): BotConfig {
  const out: BotConfig = Array.isArray(target) ? [...target] : { ...target };
  for (const [key, value] of Object.entries(source)) {
    if (value && typeof value === "object" && !Array.isArray(value)) {
      out[key] = deepMerge((out[key] as BotConfig) || {}, value as BotConfig);
    } else {
      out[key] = value;
    }
  }
...
export function resolveBotConfig(baseConfig: BotConfig, presetName: string | null = null): BotConfig {
...
function getHumanState(g: GameState): BotHumanState {
  if (!g._botHumanState) {
    g._botHumanState = {
...
function getLaneForX(x: number): number {
  if (x < CANVAS_W / 3) return 0;
  if (x < (CANVAS_W * 2) / 3) return 1;
  return 2;
}

function updateHumanCursorState(state: BotHumanState, tick: number): void {
...
function scheduleHumanAim(
  state: BotHumanState,
  human: BotConfig,
  point: ThreatPoint,
  desiredAim: { x: number; y: number },
  tick: number,
  rng: () => number,
): void {
...
function humanizeThreats(
  g: GameState,
  threats: ThreatPoint[],
  human: BotConfig,
  tick: number,
): { state: BotHumanState; perceived: ThreatPoint[] } {
  const rng = getRng();
  ...
  for (const threat of threats) {
    ...
    const seenTick = state.seenTicks.get(threat.targetRef) ?? tick;
    const reactionDelay =
...
function applyHumanAim(point: ThreatPoint, human: BotConfig): { x: number; y: number } {
  const rng = getRng();
...
function sampleWaypoints(waypoints: { x: number; y: number }[], pathIndex: number): { x: number; y: number } {
...
function getThreatReservationLimit(threat: ThreatPoint): number {
  if (threat?.isMirv) return 2;
  return 1;
}

interface BotReservation {
  targetRef: Threat;
  untilTick: number;
}
...
function getActiveBotReservations(
  g: GameState & { _botTargetReservations?: BotReservation[] },
  tick: number,
): BotReservation[] {
...
export function reserveBotTarget(
  g: GameState,
  targetRef: Threat | null | undefined,
  untilTick: number,
  tick: number,
): void {
  if (!targetRef || !Number.isFinite(untilTick) || untilTick <= tick) return;
  const reservations = getActiveBotReservations(g as GameState & { _botTargetReservations?: BotReservation[] }, tick);
...
export function botDecideAction(
  ...
  lastFireTick: number,
  tick: number,
): (ThreatPoint & { x: number; y: number }) | null {
  ...
  function isSafeFromPlanes(point: { x: number; y: number }): boolean {
  ...
  function finalizeAim(point: ThreatPoint): (ThreatPoint & { x: number; y: number }) | null {
    ...
    const rawResult = human ? prepareHumanAim(humanState!, human, point, tick, rng) : { x: point.x, y: point.y };
    if (!rawResult) return null;
    const result: ThreatPoint & { x: number; y: number } = { ...point, ...rawResult };
  ...
  if (hasUrgentThreat && tick - lastFireTick >= cooldown) {
    ...
    for (const ut of urgentThreats) {
      if (!ut.isMirv && (ut.coveredBy ?? 0) > 0) continue;
...
export function botDecideUpgrades(
  g: GameState,
  config: BotConfig,
): { repairs: Array<{ type: string; index?: number; key?: string }>; priority: string[] } {
  const repairs: Array<{ type: string; index?: number; key?: string }> = [];
  ...
  if (config.upgradeStrategy === "random") {
    const rng = getRng();
    const pool = (config.upgradePriority as string[]).filter((key: string) => {
