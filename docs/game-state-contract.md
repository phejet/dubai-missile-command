...
mutations use `addScore`, including zero-point automation kills, bonuses, penalties
and spending. `setScoreAuditSink` is an optional module-level testing hook; it is not
serialized game state. Kill stats still count automation kills.

Player, F-15, EMP and flare kills earn base reward × current combo. Hornets, Roadrunner,
Patriot, Iron Beam, Phalanx, impact and friendly-fire explosions earn zero kill points.
