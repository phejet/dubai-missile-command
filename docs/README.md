...
- [`replay-capture-assembly-plan.md`](./replay-capture-assembly-plan.md) — the capture envelope: schema, run/capture identity, the D1-shaped summary projection, size degradation ladder, and the `/api/save-capture` dev-parity contract. Roadmap step 3, shipped.
- [`capture-worker-backend-plan.md`](./capture-worker-backend-plan.md) — planned Cloudflare Worker + R2 + D1 backend for captures: two-table schema, ingest ladder, abuse controls, contract-parity strategy, and the curl gate. Roadmap step 5, not started.
