...
test.describe("Smoke tests", () => {
  ...
  test("phone keeps its flavor badge without a gameplay upload badge", async ({ page }) => {
    await page.setViewportSize({ width: 320, height: 740 });
    await page.evaluate(() => {
      document.querySelector<HTMLElement>("#game-shell")!.dataset.appFlavor = "staging";
    });
    await expect(page.locator(".app-flavor-badge")).toBeVisible();
    await expect(page.locator("#capture-upload-indicator")).toHaveCount(0);
  });
  ...
  test("opens run recap from game over", async ({ page }) => {
    ...
    await expect(page.locator(".run-recap__feature")).toBeVisible();
    await expect(page.locator(".run-recap__feature-caption")).toContainText(/final stand/i);
    await expect(page.getByText(/final waves/i)).toBeVisible();
    await expect(page.locator(".run-recap__pill")).toHaveCount(1);
    await expect(page.locator(".run-recap__pill--terminal")).toHaveCount(1);
